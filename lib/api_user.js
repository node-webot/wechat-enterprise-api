 ...
 * 根据Code获取用户ID
 *
 * 详情：http://qydev.weixin.qq.com/wiki/index.php?title=OAuth验证接口
 * Examples:
 * ```
...
exports._getUserIdByCode = function (code, callback) {
  ...
  var url = this.prefix + 'user/getuserinfo';
  var info = {
    access_token: this.token.accessToken,
    code: code
  };
  url += '?' + querystring.stringify(info);
 ...
 * 根据Code获取用户ID
 *
 * 详情：http://qydev.weixin.qq.com/wiki/index.php?title=成员登录授权
 * @param {String} code LoginPage授权获取的code
 * @param {Function} callback 回调函数
 */
exports.getLoginUserInfoByCode = function (auth_code, callback) {
  this.preRequest(this._getLoginUserInfoByCode, arguments);
};

/*!
 * 获取登陆成员信息未封装版本
 */
exports._getLoginUserInfoByCode = function (auth_code, callback) {
 ...
 * @param {String} redirect 授权后要跳转的地址
 * @param {String} state 开发者可提供的数据
 * @param {String} usertype 支持登录的类型，有member(成员登录)、admin(管理员登录)、all(成员或管理员皆可登录)，默认值为admin
 */
exports.getLoginURL = function (redirect, state, usertype) {
  var url = 'https://qy.weixin.qq.com/cgi-bin/loginpage';
  var info = {
    corp_id: this.corpid,
    redirect_uri: redirect,
    state: state || ''
